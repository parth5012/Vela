...
export default function ChatScreen() {
  ...
  React.useEffect(() => {
    if (activeThreadId) {
      ...
      const hasAuthError = threadMsgs.some(
        (m) => m.role === 'assistant' && m.content.includes('Google Workspace not connected')
      );
      if (hasAuthError && !authRequired[activeThreadId]) {
        setAuthRequired(prev => ({ ...prev, [activeThreadId]: true }));
      } else if (!hasAuthError && authRequired[activeThreadId]) {
        setAuthRequired(prev => ({ ...prev, [activeThreadId]: false }));
      }
    }
  }, [activeThreadId, messages, authRequired]);;
  ...
  const [selectedAgent, setSelectedAgent] = useState(defaultPersona || 'personal assistant');
            ...
            {showRawMap[item.id] ? (
              ...
              <View style={{ gap: 4, width: '100%' }}>
                {bubbleContent.map((segment, idx) => {
                  ...
                  {item.content.includes("Google Workspace not connected") && (
                    <Pressable
                      style={styles.inlineAuthButton}
                      onPress={() => router.push('/settings')}
                    >
                      <Text style={{ fontSize: 14 }}>🔑</Text>
                      <Text style={[styles.inlineAuthButtonText, { fontSize: sizes.sub }]}>Connect Google Account</Text>
                    </Pressable>
                  )}
                {(() => {
  ...
  const handleSendWelcome = useCallback(async (textToSend: string, personaId?: string) => {
    ...
    if (!apiUrl || !apiKey) {
      Alert.alert('Configuration Required', 'Please configure API URL & Key in Settings.');
      return;
    }
    Keyboard.dismiss();
    ...
    await streamAgentResponse(
      ...
      (error) => {
        ...
        delete abortControllersRef.current[newThreadId];
        cleanUpThrottleAndHeal(newThreadId);
        const errMsg = error?.message || (typeof error === 'string' ? error : '') || 'Failed stream response.';
        appendToken(
          newThreadId,
            `\n\n⚠️ **Error:** ${errMsg}`
          );
  ...
  return (
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.personaScrollContainer}>
      ...
      {/* Unifying Input container at the bottom */}
        {activeThreadId && authRequired[activeThreadId] && (
    <Pressable
      style={[styles.authChip, { backgroundColor: colors.card, borderColor: colors.border }]}
      onPress={() => router.push('/settings')}
    >
      <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1, marginRight: 8 }}>
        <Text style={{ fontSize: 16, marginRight: 8 }}>🔑</Text>
        <Text
          style={[styles.authChipText, { color: colors.text }]}
          numberOfLines={1}
        >
          Google Workspace connection required
        </Text>
      </View>
      <View style={[styles.authChipButton, { backgroundColor: accentHex }]}>
        <Text style={styles.authChipButtonText}>Connect</Text>
      </View>
    </Pressable>
  )}
